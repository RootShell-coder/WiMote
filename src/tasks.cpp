...
void publishSystemChanges(JsonObject system)
{
  ...
  {
    publishMessage("system/heap_free", String(heap_free).c_str(), true);
  ...
  {
    publishMessage("system/heap_size", String(heap_size).c_str(), true);
  ...
  {
    publishMessage("system/cpu_freq_mhz", String(cpu_freq).c_str(), true);
  ...
  {
    publishMessage("system/sdk_version", sdk_version.c_str(), true);
  ...
  {
    publishMessage("system/flash_size", String(flash_size).c_str(), true);
...
void publishWiFiInfo(JsonObject wifi)
{
  ...
  {
    publishMessage("wifi/status", status.c_str(), true);
    last_status = status;
  }
  ...
  if (status == "connected")
  {
    ...
    if (ssid != last_ssid)
    {
      publishMessage("wifi/ssid", ssid.c_str(), true);
      last_ssid = ssid;
    }
    ...
    if (ip != last_ip)
    {
      publishMessage("wifi/ip", ip.c_str(), true);
      last_ip = ip;
    }
    ...
    if (rssi != last_rssi)
    {
      publishMessage("wifi/rssi", String(rssi).c_str(), true);
      last_rssi = rssi;
    }
    ...
    {
      publishMessage("wifi/channel", String(channel).c_str(), true);
